...
Cases with `expected.failureClass` never call either model endpoint. The runner locally executes only the fixed allowlist `malformed`, `timeout`, `quota`, `refusal`, and `network`, taking the actual class from `execution.scenario` and comparing it with the expected class. A missing, unknown, or mismatched declaration fails the failure-handling gate. This keeps non-semantic resilience checks reproducible without allowing them to inflate model coverage, placement accuracy, or live-evidence counts. Every other case still runs against the configured model service.
